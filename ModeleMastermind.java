public class ModeleMastermind {
     ...
     */
    private int nbValeurs = 6;

    /**
     * getTaille de la combinaison
     */
    ...
    public ModeleMastermind() {
        this.combinaison = new int[getTaille()];
    }
    
    public ModeleMastermind(int taille, int nbCouleurs) {
    	 this.combinaison = new int[taille];
        this.taille = taille;
        this.nbValeurs = nbCouleurs;
    }

    /**
     * genere aleatoirement une combinaison de getTaille getTaille dont les valeurs
     * sont comprises entre 0 et getNbValeurs-1
     */
    public void genererCombinaison() {
        for (int i = 0; i < getTaille(); i++) {
            this.combinaison[i] = (int) (getNbValeurs() * Math.random());
        }
    }
    ...
    public int nbChiffresMalPlaces(int tabChiffres[]) {
        int v = 0;
        int combinaisonAux[] = new int[getTaille()];
        int tabChiffresAux[] = new int[getTaille()];
        for (int i = 0; i < getTaille(); i++) {
        ...
        for (int i = 0; i < getTaille(); i++) {
            ...
            for (int j = 0; j < getTaille() && !trouve; j++) {
